...
extern int alert_box_ext( char *title, unsigned int options, char *fmt, ... );
extern void do_error( char *file );
extern int valert_box_ext( char *title, unsigned int options, char *fmt, va_list marker );
...
extern short num_print_name( FILE *stream, char *head, NAME_NUMBER *np, NAME_REC *nmp );
extern short num_report_print( FILE *stream, CHART_INFO *cip,
		    NUMBERS *np, NAME_NUMB_GROUP *nngp, short head );
extern short pers_print_name( FILE *stream, char *head, NAME_NUMBER *np, NAME_REC *nmp, short file );
