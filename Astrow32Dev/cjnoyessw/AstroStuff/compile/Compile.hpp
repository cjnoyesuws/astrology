...
public:
	void SetWizFileInfo( XPFILE_DB &info ) { wizfileinfo = info; } 
      ...
      ChartCompiler(CWnd *pPar, enum ChartType tp, BOOL wiz=FALSE)  { pParw = pPar; chartType = tp; stream = NULL; out_file_ptr =NULL; graph_ptr = NULL; bWizard=wiz;}
...
public:
      NatalChartCompiler(CWnd *pPar, enum ChartType tp, BOOL wiz=FALSE) : ChartCompiler(pPar,tp,wiz) {}
...
public:
      TransChartCompiler(CWnd *pPar, enum ChartType tp, BOOL wiz=FALSE) : ChartCompiler(pPar,tp,wiz) {}
...
public:
      ReturnChartCompiler(CWnd *pPar, enum ChartType tp, BOOL wiz=FALSE) : ChartCompiler(pPar,tp,wiz) {}
