...
static void update_wpcs( unsigned *wpcs, WPI *wpi, int count )
{
 int i;
...
void wp5_code_page_update( unsigned *wpcs )
{
 int (IMPORT *kbp)(void);
 UINT page;
 ...
 page = GetOEMCP();
#else
 kbp = GetProcAddress(GetModuleHandle("KEYBOARD"),"GETKBCODEPAGE");
 page = kbp();
#endif	 
 switch( page ) {
       case 850 :
