...
int harmonic_print_aspect_entry( FILE *stream, NUM planet, ASPEC *ptr,
        char *name, int file, char *rpt )
{
 int e;
...
int harmonic_print_midpoint_entry( FILE *stream, NUM planet, MIDPT *ptr,
 ...
 if ( ptr->no_aspect > 0 ) {
    ...
    if ( ptr->no_aspect ) {
	  ...
	  return( 1 );
	   ...
	   if ( harmonic_print_aspect_entry( stream, planet, as, name, -1, (char *)rpm ) )
	      return( 1 );
	   }
       }
    fputc( '\n', stream );
...
int harmonic_print_entry( FILE *stream, AS_INF *inf, NUM planet, BITS mode,
     char *name, int fil, int hsfil, int apfil,
	 int table, int global )
{
 COUNT i;
 ASPEC *as;
 char ps[25];
 ...
 if ( output_planet_sign( stream, planet, inf,
			-1, NULL, mode, name, name, (char *)rppl ) )
    return( 1 );
 if ( mode & TEXT ) {
    if ( inf->cusp == -1 ) {
       harmonic_text( fil, fil, inf->sign, planet, -1, stream );
       if ( inf->retrograde )
 ...
 if ( mode & HOUSES ) {
  ...
  if ( mode & ASPECTS ) {
    if ( inf->no_aspects > 0 ) {
       if ( output_title( stream, name, name, "", (char *)rpah ) )
	  return( 1 );
	for ( as = inf->aspectr, i = 1; i <= inf->no_aspects; ++i, ++as )
	  if ( harmonic_print_aspect_entry( stream, planet, as, name, apfil, (char *)rpas) )
	      return( 1 );
       }
    }
 if ( inf->no_midpt > 0 ) {
   	...
   	harmonic_print_midpoint_entry( stream, planet, mid, mode, -1, name, table, global );
    }
 fputc( '\n', stream );
 return( 1 );
}
...
void harmonic_print_results( FILE *stream, AS_INF *inf_ptr, BIRTH_DB *data,
              BITS mode,  AS_INF *hinfo,
                           NUM house_mode, DATA_PACKET *dpk, char *comment, int interp )
{
 ...
 if ( mode & TEXT ) {
    if ( mode & ASPECTS )
      apfile = open_asp_files( ASPECT_FILE );
 ...
 if ( rphs == NULL || rpas == NULL || rpah == NULL || rppl==NULL || rpmsc==NULL || ferror( stream ) ) {
  ...
  for ( i = 0, inf = inf_ptr, hinf = hinfo; i <= maxp; ++i, ++inf, ++hinf ) {
     if ( x_kb_check() )
	   break;
	 harmonic_print_entry( stream, inf, i, mode, "Natal", pfile, hfile, apfile, table, global);
	 harmonic_print_entry( stream, hinf, i, mode, "Harmonic", pfile, hfile, apfile, table, global);
     if ( ferror( stream ) ) {
        goto er_close2;
        }
     }
