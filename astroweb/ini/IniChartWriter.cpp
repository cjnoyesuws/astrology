...
int IniChartWriter::writeLine(const char *key, const char *buff) {
	return fprintf(stream, "%s=%s\n", key, buff );
}

int IniChartWriter::writeSection(const char *key) {
	return fprintf(stream,"[%s]\n",key);
}

int IniChartWriter::writeHouses(const char *sec, short *houses) {
	writeSection(sec);
	for (int i =0; i < 12; i++) {
...
int IniChartWriter::writeAspect(const char *key, int planet, ASPEC &aspect ) {
	return fprintf(stream,"%s=\"%d,%d,%d,%d\"\n", key, planet, aspect.planet, aspect.aspect, aspect.orb );
}

int IniChartWriter::writePlanet(const char *key, int index, AS_INF &data) {
...
int IniChartWriter::writeAspects(const char *key, int index, int planet, AS_INF &data ) {
   char buf[30];
...
int IniChartWriter::writeAspects(const char *sec, AS_INF *data, int count) {
	int index = 0;
	writeSection(sec);
...
int IniChartWriter::writePlanets(const char *sec, AS_INF *data, int count, int start) {
	char buf[30];
...
int IniChartWriter::writeNatal(int maxpt) {
...
int IniChartWriter::writeCompat(int maxpt) {
	writeNatal(maxpt);
...
int IniChartWriter::writeProgr(int maxpt) {
	writeNatal(maxpt);
	writeHouses("OtherHouses", alt_cusps);
...
int IniChartWriter::writeTrans(int maxpt) {
	writeNatal(maxpt);
	writeTransitDates();
...
int IniChartWriter::writeTransitDates() {
...
int IniChartWriter::writeTransitChart(int index) {
	char buf[30];
