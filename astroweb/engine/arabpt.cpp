...
int do_arabic_part( int i, AS_INF *arab, unsigned short mode, int year  )
{
 long l;
 int aspects, maxp;
 ASPEC *asp;
 ...
 misc_calc( arab, mode, year, house_cusps[0] );
 if ( mode & ASPECTS ) {
    asp = new ASPEC[30];
    if ( asp == NULL ) {
       errno = ENOMEM;
    ...
    arab->aspectr = asp;
    aspects = do_arab_aspect( arab, (AS_INF *)birth_inf, 0, mode, maxp );
