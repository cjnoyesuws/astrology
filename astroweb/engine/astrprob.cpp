...
const char * REPORT_NAME=	"PROBLEM";
...
ASTR_PROBLEM problems[N_ASTR_PROB+2] = {
...
static char prob_result[N_RESULT][RES_LEN] = {"Abruptness",
...
int output_problem( FILE *stream, ASTR_PROBLEM *probp, char *s1, char *s2, char *rpt, int table )
{
 ...
 do {
    ...
    switch( *p++ ) {
	  ...
	  case '_' :
	       ...
	       break;
	  case '1' :
	       opcaps( stream, planet_str(probp->planet), buf );
	       cap = 0;
	       break;
	  case '2' :
	       opcaps( stream, planet_str(probp->arg2), buf );
	       break;
	  case '3' :
	       switch( probp->arg_type ) {
		     case HOUSE :
		     case HSECSP :
			  opnum( stream, 2, probp->arg2 );
			  break;
		     case SIGNHSE :
		     case PLAHSE :
			  opnum( stream, 2, probp->arg3 );
			  break;
		     default :
			  break;
		     }
	       break;
	  case '4' :
	       opcaps( stream, aspect_str(probp->arg3), buf );
	       break;
	  case '5' :
	       if ( probp->arg_type == PLASIGN ) {
		  opcaps( stream, sign_str(probp->arg3), buf );
		  }
	       else {
		  opcaps( stream, sign_str(probp->arg2), buf );
		  }
	       break;
	  case '6' :
	       switch( probp->arg_type ) {
		     case HOUSE :
		     case HSECSP :
			  opcaps( stream, number_str(probp->arg2), buf );
			  break;
		     case SIGNHSE :
		     case PLAHSE :
			  opcaps( stream, number_str(probp->arg3), buf  );
			  break;
		     default :
			  break;
		     }
	  ...
	  case '7' :
	       if ( probp->result >= 5 ) {
		  opcaps( stream, tb_get_text( table,
			    ST_PROBRES, probp->result - 5 ), buf );
		  }
	       break;
	  case '8' :
	  case '9' :
	       p = out_bool_query( stream,probp->result==AND, p );
	       cap = 0;
	       break;
	  case 'A' :
	  case 'a' :
...
int print_a_problem( FILE *stream, ASTR_PROBLEM *probp, char *s1, char *s2, int table )
{
 char *res;

 output_problem( stream, probp, s1, s2, (char *)rps[probp->arg_type-1], table);
 if ( probp->result == AND ) {
    ++probp;
    return( print_a_problem( stream, probp, s1, s2, table ) );
    }
 else if ( probp->result == OR ) {
    while ( probp->result == OR )
	  probp++;
    }
 output_problem( stream, probp, s1, s2, (char *)rps[7], table);
#ifndef NDEBUG
 if ( probp->result < 5 )
    error_state = 1;
 else
...
void problem_solving_report( FILE *stream, AS_INF *inf_ptr,
    BIRTH_DB *data, BITS mode, NUM house_mode, DATA_PACKET *dpk, char *comment )
{
 FILE *rstream;
 const char *report;
 char *fi;
 ASTR_PROBLEM *problems;
 long l;
 int i, j, k, r;
 int global, table;
 static int text_items[] = {CHPR_HOUSE,CHPR_ASPECT,CHPR_SIGN,CHPR_SIGN_HOUSE,
	  CHPR_HSE_CUSP_SIGN,CHPR_PLAN_HOUSE,CHPR_PLAN_SIGN, CHPR_RESULT};

 fi = set_path( "PROBLEM.DAT", DATA_F );
 if ( ( rstream = fopen( fi, "rb" ) ) == NULL ) {
    do_error( fi );
    return;
    }
 l /= sizeof( ASTR_PROBLEM );
 problems = new ASTR_PROBLEM[l];
 if ( problems == NULL ) {
    fclose( rstream );
    errno = ENOMEM;
    do_mem_error( 0 );
 ...
 if ( fread( problems, sizeof( ASTR_PROBLEM ), (int) l, rstream ) < (int)l ) {
    delete problems;
    fclose( rstream );
    do_error( fi );
    return;
 ...
 for (i = 0; i < 8; i++ ) {
     if ( ( rps[i] =
	   get_report( table, global, text_items[i] ) ) == NULL ) {
	delete problems;
	return;
	}
     }
 ...
 i = 0;
 do {
    if ( (r = solve_problem( inf_ptr, &problems[i], i, &k, &j )) ) {
#ifndef NDEBUG
       if (error_state || (r  && r < 5) ) {
	  fprintf( stream, "ERROR RESULT!! r = %d, i = %d, k = %d, j = %d\n" );
	  fprintf( stream, "problems[j] p=%d, t=%d, a2=%d, a3=%d, r=%d\n",
	     problems[j].planet, problems[j].arg_type, problems[j].arg2,
		problems[j].arg3, problems[j].result );
	  fprintf( stream, "problems[i] p=%d, t=%d, a2=%d, a3=%d, r=%d\n",
	     problems[i].planet, problems[i].arg_type, problems[i].arg2,
	     problems[i].arg3, problems[i].result );
	  fprintf( stream, "problems[i+1] p=%d, t=%d, a2=%d, a3=%d, r=%d\n",
	     problems[i+1].planet, problems[i+1].arg_type, problems[i+1].arg2,
	     problems[i+1].arg3, problems[i+1].result );
	  fprintf( stream, "error_state = %d", error_state );
	  fprintf( stream, "est[] = %d, %d, %d, %d, %d, %d\n",
	    est[0], est[1], est[2], est[3], est[4], est[5] );
	  error_state = 0;
	  }
#endif
      print_a_problem( stream, &problems[j], data->name, comment, table );
	  fputs("\n\n", stream );
      }
#ifndef NDEBUG
    if (error_state || (r  && r < 5) ) {
