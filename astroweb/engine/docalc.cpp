...
void misc_calc( AS_INF *ptr, unsigned int code, short year, short asc_min )
{
 ...
 ptr->graph_minutes = graph_minutes(ptr->minutes_total,asc_min);
 format_degs(ptr->minutes_total,ptr->formatted);
}
...
/*	   case PORPHYRYX :
 ...
 for ( i = ASCEND, inf_ptr = info; i <= endplan; ++i, ++inf_ptr )
     misc_calc( inf_ptr, mode, data->noon_date.year, info->minutes_total );
 if (mode & HOUSES) {
    for ( i = 0; i < 12; i++ ) {
        house_inf[i].minutes_total = house_cusps[i];
    }
    for (i = 0;  i < 12; i++ ) {
        misc_calc( &house_inf[i], mode, data->noon_date.year,house_inf->minutes_total );
    }
 }
 if ( mode & ASPECTS ) {
 ...
 setup_signs(house_inf[0].minutes_total);
 if ( !(mode & HOUSES ) )
