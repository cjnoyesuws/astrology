...
int solar_print_grid_entry( FILE *stream, AS_INF *inf, NUM planet, BITS mode,
     char *name, int table, int global, char *which)
{
 COUNT i;
 ASPEC *as;
 char ps[25];
...
void solar_print_grid( FILE *stream, AS_INF *inf_ptr, BIRTH_DB *data,
              BITS mode,  AS_INF *sinfo, int type,
...
// rpsrad = get_report( table, global, CHSO_RADIX );
 ...
 if ( rphs == NULL || rpah == NULL || rppl==NULL || rpmsc==NULL || ferror( stream ) ) {
  ...
  for ( i = 0, inf = inf_ptr; i <= maxp; ++i, ++inf ) {
     if ( x_kb_check() )
	   break;
     solar_print_grid_entry( stream, inf, i, mode, data->name, table, global, "Birth" );
     if ( ferror( stream ) ) {
        goto er_close2;
        }
     }
  ...
  fputs("[SOLAR DIRECTED POSITIONS]@tb\x80", stream);
  for ( i = 0, inf = inf_ptr, sinf = sinfo; i <= maxp; ++i, ++inf, ++sinf ) {
     if ( x_kb_check() )
	   break;
	 solar_print_grid_entry( stream, sinf, i, mode, "Solar Directed", table, global, "Solar Directed" );
     if ( ferror( stream ) ) {
        goto er_close2;
        }
     }
 fputs("@tb\n\n", stream );
 if ( mode & ASPECTS ) {
	output_title( stream, data->name, "Solar Directed", comment, (char *)rpah );
	AspectGridStr *pGrid2 = new AspectGridStr(maxp,grddp, type,mode);
    pGrid2->MakeGrid(sinfo,maxp);
