...
#define CONSULTATION_GRAPH_FILE LAST_NON_GRAPH + CONSULTATION_FILE
...
#define CONSULTATION_GRID_FILE LAST_GRAPH + CONSULTATION_FILE
...
struct O_LOC_DB {                /* basic location database structure */
	...
	short  min_offset;
					 ...
					 /* New Format Expanded */
struct LOC_DB { 	       /* basic location database structure */
	...
	char tzcodekey[5];
					 ...
					 /* New Format Expanded */
struct BIRTH_DB {	       /* birth database structure */
	...
	char tzcodekey[5];
				       ...
				       /* Old format */
struct O_BIRTH_DB {              /* birth database structure */
	...
	char filler[16];     /* for earlier version file compatibility */
				       ...
				       /* Old format */
struct O_FILE_DB {               /* file database structure */
	char name[O_NAME_WIDTH+1];
...
struct W16_FILE_DB {	       /* file database structure */
...
struct FILE_DB {	       /* file database structure */
	char name[NAME_WIDTH+1];       /* this has been manually padded */
	char comment[COMMENT_WIDTH+1];
	char file_name[255];
	short type;
 ...
 struct XPFILE_DB {	       /* file database structure */
	char name[NAME_WIDTH+1];       /* this has been manually padded */
	char comment[COMMENT_WIDTH+1];
	char file_name[255];
	short type;
...
struct GRAPH_MINUTES {
    short minutes;
    short graph_minutes;
};


extern BIRTH_DB birth_rec, comp_rec, trans_rec, current_rec;
extern LOC_DB reloc_data;
extern GRAPH_MINUTES signs[];
