...
{
 if ( fatal )
    FatalAppExit(0, /*CResStr(RESSTR_E_1)*/"Fatal Out of Memory Error!" );
 alert_box_ext( ERROR2, MB_OK|MB_ICONSTOP, /*CResStr(RESSTR_E_2)*/"Out of Memory Error!" );
}

#define NON_FATAL 0

static char buf[500];
static char a_title[] = "A Message from ASTRO";
...
{
 int result;
 char *a_title = /*CResStr(RESSTR_E_MSG)*/ "A Message from ASTRO";
 HWND handle;
 va_list marker;
...
void do_error( const char *file )
{
 ...
 if ( errno == ENOMEM ) {
    ...
    return;
 if ( errno > 40 )
    alert_box_ext( ERROR2, MB_OK|MB_ICONSTOP, /*CResStr(RESSTR_E_ILL)*/ "ILLEGAL ERROR #%d--%s", errno, /*CResStr(RESSTR_E_CAL)*/spmsg );
 else if ( errors[errno] == NULL )
    alert_box_ext( ERROR2, MB_OK|MB_ICONSTOP, /*CResStr(RESSTR_E_UND)*/ "UNDEFINED ERROR #%d--%s", errno, /*CResStr(RESSTR_E_CAL)*/spmsg );
 else if ( errors[errno] == stdmsg )
    alert_box_ext( ERROR2, MB_OK|MB_ICONSTOP, /*CResStr(RESSTR_E_STD)*/ stdmsg, file, errno, /*CResStr(RESSTR_E_CAL)*/ spmsg );
 else
    alert_box_ext( ERROR2, MB_OK|MB_ICONSTOP, "%s : %s ", /*CResStr(*/errors[errno]/*)*/, file );
 errno = 0;
}
