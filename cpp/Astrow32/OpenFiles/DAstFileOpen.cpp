...
BOOL CDAstFileOpen::OnInitDialog()
{
	...
	m_list.InsertColumn(0,"Name", LVCFMT_LEFT,276,-1);
	m_list.InsertColumn(1,"Comments", LVCFMT_LEFT,256,1);
	m_list.InsertColumn(2,"File Type", LVCFMT_LEFT,256,2);
	m_list.InsertColumn(3,"File Name", LVCFMT_LEFT,150,3);
	m_reportbmp.LoadBitmap(IDB_TEXT);
	m_graphbmp.LoadBitmap(IDB_GLYPH);
...
void CDAstFileOpen::LoadList()
{
  ...
  if ( !m_filelist.IsEOF() ) {
	  while ( !m_filelist.IsEOF() ) {
		  ...
		  m_list.SetItemText(nItem,3,m_filelist.m_filename);
