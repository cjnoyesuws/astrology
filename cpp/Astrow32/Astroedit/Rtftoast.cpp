...
typedef struct CWDOX {
	char name[30];
	char *out;
	int (*df)( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
	} CWDOX;
...
static int set_win( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int rtf_read( FILE *stream, CODE_WORD *cwp );
static int skip_dest( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int set_caps( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int set_inv( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int output_bold( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int output_head( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int output_us( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int plain( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int font( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int output( FILE *in, strstream &out, CODE_WORD *cwp, char *s );
static int do_code( CODE_WORD *cwp, FILE *in, strstream &out );
...
static int pop_state( strstream &out )
{
 STATE tos;
...
typedef int (__cdecl *matchfunc)(const void *,const void *); 

static int do_code( CODE_WORD *cwp, FILE *in, strstream &out )
{
 CWDOX *cwd;

 cwd = (CWDOX *)lfind( cwp->name, cwd_array, (unsigned int *)&cwd_count, sizeof( CWDOX ), (matchfunc)strcmpi );
 if ( cwd == NULL )
    return( 0 );
...
int RichTextToAstro( char *file, strstream &out)
{
 FILE *instream;
 CODE_WORD cw;
 int c, d, e, s;
 
 if ( ( instream = fopen( file, "rt" ) ) == NULL ) {
    alert_box( "ERROR!!", "Can't Open %s For Reading!\n", file );
    return( 2 );
    }
       ...
       switch( c ) {
	     ...
	     default :
		  if ( !curr_state.map_inv ) {
			...
			out.put( (unsigned char)toupper( c ) );
		     else if ( isascii( c ) )
...
static int output_checkstate( FILE *in, FILE *out, CODE_WORD *cwp, char *s );
...
static CWDO cwd_array[] = { "par", "\n", output_checkstate, "b", "", output_bold,
   "i", "", output_us, "ul", "", output_us, "ulw", "", output_us,
   "uld", "", output_us, "uldb", "", output_us, "plain", "", plain,
   "plain", "", plain, "lquote", "\'", output, "rquote", "\'", output,
   "ldblquote", "\"", output, "rdblquote", "\"", output,
...
typedef int (__far __cdecl *matchfunc)(const void *,const void *); 
...
int RichTextToAstro( char *infile, FILE *outstream)
{
 FILE *instream;
 CODE_WORD cw;
 int c, d, e, s;
 ...
 if ( ( instream = fopen( infile, "rt" ) ) == NULL ) {
    alert_box( "RTFTOAST", "Can't Open %s For Reading!", infile );
    return( 2 );
    }
 if ( setvbuf( instream, NULL, _IOFBF, 4096 ) ) {
    if ( setvbuf( instream, NULL, _IOFBF, 2048 ) ) {
