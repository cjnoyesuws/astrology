...
********************************************************************/
DWORD FAR PASCAL TPHVersion (VOID)
{
...
********************************************************************/
DWORD FAR PASCAL TPHCompatibility (ATOM AppName, WORD AppVersion)
{                          
...
********************************************************************/
DWORD FAR PASCAL TPHTimeStamp (VOID)
{                                                                  
	return 0;
}/* End TPHTimeStamp */	     
...
********************************************************************/
WORD FAR PASCAL TPHEntry (WORD wLanguage)
{   
...
********************************************************************/
int FAR PASCAL TPHHandleTokenValReturn (LPTPH_RETURNVAL 	lpRetData,
								HWND		hLinkWnd,
								WORD		msg)
{
	...
	if( lpRetData->lpTokenData->wTokenID  == MENULOAD_TKN)
	{
		...
		lpfnWpidMenuItemInsert(MenuHandle, WP_InsertMenu, WPMF_BYPOSITION, (WORD)0, ASTROReport, 0, NULL, 0);
	}

	if( lpRetData->lpTokenData->wTokenID  == ASTROReport)
	{
		/*Enter the code that will be called when your menu or button is selected*/
		MessageBox(NULL, "ASTROReport", "Menu", MB_OK);
...
/*FUNCTION***********************************************************
	...
	RETURNS:  See return values documented in WOAPI
********************************************************************/
DWORD FAR PASCAL TPHValidateToken (LPMAC_TOKEN lpTokenData, LPVOID lpDataType)
{   

/*	If creating your own menu items, validate them using the commented code below.

	if (lpTokenData->wTokenID == ???)
	{		
								...
								BOOL bOverwrite)
{
	...
	hBitmap = 0;
	lpfnW6azTowz(szThetaName, ThetaName);
