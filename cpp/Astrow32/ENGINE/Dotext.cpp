...
short close_files_old( short file_num )
{
 int er = 0;
 TXTFIL *pt;
...
void *x_read_fil( int ofs, short file_num, TFIL *p )
{
 char *buf, *bp;
...
short x_open_files_old( char *base_name, short num )
{
 short file_num = 0, flag;
 int h1, h2;
 char *fi, name[15];
 TXTFIL *pt;
 ...
 if ( pt->txptp.is_open != MEMORY && pt->txptp.is_open != DISCARDABLE ) {
    ...
    strcat( name, ".NDX" );
    fi = set_path( name, DATA_F );
    assert( fi != NULL && strlen( fi ) < 150 );
    strcpy( pt->txptp.name, fi );
    if ( ( h1 = open( fi, O_BINARY|O_RDONLY) ) == -1 ) {
       do_error( fi );
       return( -1 );
       }
 ...
 if ( pt->textp.is_open != MEMORY && pt->textp.is_open != DISCARDABLE ) {
    ...
    if ( ( h2 = open( fi, O_BINARY|O_RDONLY ) ) == -1 ) {
       close( h1 );
       do_error( fi );
       return( -1 );
       }
 ...
 if ( pt->txptp.is_open == DISCARDABLE ) {
    ...
    pt->txptp.is_open = OPEN;
    pt->txptp.fil = h1;
    pt->ptr =(TXTPT *) x_read_fil( sizeof(short) * num, file_num, &pt->txptp );
...
TXTPT *get_text_ptr( short col, short row, short deep, TXTFIL *pt )
{
 ...
 if ( pt->ptr == NULL && pt->txptp.is_open != MEMORY ) {
    offset = (ofs * sizeof( TXTPT )) + pt->mode * sizeof( short );
...
short get_transfer_text_asp_old( short row, short col, short deep,
                                         short file_num, FILE *stream )
{
 ...
 if ( pt->text == NULL ) {
    ...
    if ( ( nr = fwrite( (char *)(pt->text + ptr->starts_at), sizeof( char ),
...
const char *get_text_asp_old( short row, short col, short deep,
                                         short file_num)
{
 TXTPT *ptr;
 int reads, rem, x, lsiz, nr;
 ...
 if ( ptr->length < 1 )
    ...
    return( (char *)(pt->text + ptr->starts_at) );
