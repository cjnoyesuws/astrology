...
void do_returns( FILE *stream, BIRTH_DB *birth, RETURN_DATA *rd, int table, CPtrArray *pDataArray )
{
...
void return_chart( FILE *stream, int pl, BIRTH_DB *birth, BITS mode, NUM house_mode,
   DATA_PACKET *dpk, char *comment, int grid, CPtrArray *pDataArray, BIRTH_DB *with )
{
 char buf[25];
 int table = -1, global = -1;
 const char *report;
 char rbuf[25];
 strcpy( rbuf, REPORT_NAME );
 if ( grid )
	 strcat(rbuf,"GRID");
 pRec = with;
 table = get_tables( rbuf, &global );
 if ( table == -1 )
    return;
 strcpy( buf, planet_str(pl+1) );
 if ( ( report = get_report( table, global, CHH_TITLE ) ) == NULL ) {
    close_tables( table, global );
    return;
    }
 else
    output_return_title( stream, pl, birth->name, comment, (char *)report );
 if ( output_birth_head_group( stream, mode, birth, table, -1,
						comment, dpk, global ) ) {
    close_tables( table, global );
    return;
    }
 if ( return_print_header( stream, buf, birth, dpk->rtdp, dpk, house_mode,
	 comment, table ) ) {
    close_tables( table, global );
    return;
    }
 if ( x_kb_check() )
    return;
 do_returns( stream, birth, dpk->rtdp, table, (grid?NULL:pDataArray) );
 fputs("\n",stream);
 close_tables( table, global );
 pRec = NULL;
}
